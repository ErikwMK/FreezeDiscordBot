...
class MyBot(commands.Bot):
    ...
    async def setup_hook(self):  # external files
        extensions = ['cogs.SetupBot', 'cogs.Stats', 'cogs.gather', 'cogs.Birthdays', 'cogs.BotStatus', 'cogs.Help', 'cogs.ErikOnly']
...
class MyHelp(commands.HelpCommand):  # custom help command
    async def send_bot_help(self, mapping):
        embed = discord.Embed(title=f"{config['BOT_NAME']}", colour=discord.Colour(config["MAIN_COLOR"]))
        ...
        embed.add_field(name=f"**Description**", value=f"The {config['BOT_NAME']} is a Discord Bot made by {config['CREATOR_NAME']} to analyse the stats of any map played in 6v6", inline=False)
        ...
        embed3.set_footer(text=f"Bot made by {config['CREATOR_NAME']}", icon_url="https://images-ext-1.discordapp.net/external/DYve2PXS45DrDa9wFHlSCPGAMS0VBIAhouo5GX9dMoU/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/807602307369271306/ad65fc359ed1f554aa2f7e23cc42330c.png?width=676&height=676")
